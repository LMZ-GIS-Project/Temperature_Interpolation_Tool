...
(function(w){
    ...
    heatmap.prototype = {
        configure: function(config){
				...
				me.set("idwcells", (config.idwcells instanceof Object)?config.idwcells:null);
		...
		doIDW: function() {
			...
			var	aWidth = Math.abs(bbox.ur[0]-bbox.ll[0]);
			var	aHeight = Math.abs(bbox.ur[1]-bbox.ll[1]);	
			var cellsX = (aWidth/idwcells.width)|0;
			var cellsY = (aHeight/idwcells.height)|0;
			
			
			
			var idwData=ctx.createImageData(idwcells.width,idwcells.height);
			var data = me.get("data");
			var minCount = 1000,maxCount = 0, minX = 1000, maxX = 0, minY = 1000, maxY = 0;
			...
			// Clear canvas
			ctx.clearRect(bbox.ll[0],bbox.ur[1],aWidth,aHeight);
			...
			for (var i=0;i<cellsX;i++)
			{
				...
				//console.log(wis,wj);
					var uNorm = (u-minCount)/(maxCount-minCount);
				
					var red = uNorm*255|0;//(uNorm>=0.5)?((uNorm-0.5)*512):0;
					var green = 255*(1-Math.abs(0.5-uNorm))|0;
					var blue = (1-uNorm)*255|0;//(uNorm<=0.5)?(1-2*uNorm)*255:0;
					/*
					idwData.data[4*cellsX*j+i*4] = red;
					idwData.data[4*cellsX*j+i*4+1] = green;
					idwData.data[4*cellsX*j+i*4+2] = blue;
					idwData.data[4*cellsX*j+i*4+3] = 100;
					
					var color = "#";
					color += (red<16)?"0":"";
					color += red.toString(16)+"00";
					color +=(blue<16)?"0":"";
					color +=blue.toString(16);
					
					//console.log(color)
					var gradientIndex=0;
					// find gradient-stops for color in between
					for (var i=0; i<gradient.length-1; i++) {
						if (uNorm<gradient[i].stop) {
							gradientIndex = i;
							break;
						}
					}
					...
					*/
					// transform u into interval from 0 .. 1
					//var uNormalized = (u-gradient[gradientIndex].stop)/(gradient[gradientIndex+1].stop-gradient[gradientIndex].stop);
					//c1 = gradient[gradientIndex].value.replace(/rgb\((\d+),(\d+),(\d+)\)/g, function(rgb,r,g,b){return r+","+g+","+b;}).split(",");
					//c2 = gradient[gradientIndex+1].value.replace(/rgb\((\d+),(\d+),(\d+)\)/g, function(rgb,r,g,b){return r+","+g+","+b;}).split(",");
					
					
					//var c1 = (uNorm<0.5)?[0,0,255]:[0,255,0];
					//var c2 = (uNorm<0.5)?[0,255,0]:[255,0,0];
					var c3=[red,green,blue];
					/*for (var l=0; l<3; i++) {
						c3[l]=c1[l]+uNorm*(c2[l]-c1[l]);
					}
					*/
					var rgb = c3[2] + (c3[1] << 8) | (c3[0] << 16);
					...
					ctx.globalAlpha = 0.5;
					// create the rectangle according to cell (i,j) 
					ctx.fillRect(bbox.ll[0]+i*idwcells.width,bbox.ur[1]+j*idwcells.width,idwcells.width,idwcells.width);
					ctx.globalAlpha = 1;
			...
			acanvas.width = idwcells.width;
			acanvas.height = idwcells.height;
			var actx = acanvas.getContext("2d");
			//ctx.putImageData(idwData,bbox.ll[0],bbox.ur[1]);
			
			//ctx.drawImage(acanvas,0,0,idwcells.width-1,idwcells.height-1,bbox.ll[0],bbox.ur[1],aWidth, aHeight);
